  ...
  const descargarComprobante = async () => {
    ...
    try {
      ...
      if (recibo.transbank?.authorizationCode) {
        lines.push(`Autorizacion: ${recibo.transbank.authorizationCode}`);
      }
      if (recibo.transbank?.paymentTypeCode) {
        lines.push(`Tipo de pago: ${recibo.transbank.paymentTypeCode}`);
      }
      if (recibo.transbank?.cardNumber) {
        lines.push(`Tarjeta: ${recibo.transbank.cardNumber}`);
      }

      let y = page.getHeight() - 72;
      page.drawText('COMPROBANTE DE PAGO - TRANSBANK', {
        x: 72,
        y,
        size: 16,
        font: fontBold,
            ...
            <div className="mt-6 grid gap-3 text-xs text-slate-600">
              ...
              <div className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3">
                ...
                <p className="mt-1 font-semibold text-slate-700">
                  {formatDateTime(recibo.pedido.createdAt)}
                </p>
              </div>
              <div className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3">
